...
Man kann einfach alle Chunks durchfueheren und die finalen Datensatz in letzten Chunk finden:
...
hospitalisierung_clean <- 
  ...
  mutate(Weekday = as.factor(weekdays(Berichtsdatum))) %>%
  filter(!(Bundesland == "Bundesgebiet" | Altersgruppe == "00+")) %>%
  filter(Weekday == "星期一") %>% # "周一" oder "星期一" bedeutet "Montag"
...
impfung_clean_sum_new <- 
  ...
  filter(Weekday == "日") %>% # ”周日“ oder "日" bedeutet "Sonntag"
...
final_lag <-
  inzidenznachAnmeldedatum_hosp_impf_lock %>%
  group_by(Bundesland, Altersgruppe) %>%
  mutate(lag1_neuerkrankung = lag(Neuerkrankung),
         lag2_neuerkrankung = lag(Neuerkrankung,2)
         ) %>%
  na.omit() %>%
  select(Jahrwoche, Hospitalisierung, Neuerkrankung, lag1_neuerkrankung, lag2_neuerkrankung, Bundesland, Altersgruppe, Jahreszeit, Impfquote, Lockdown) %>%
  filter(Impfquote != 0)
summary(final_lag)
```
