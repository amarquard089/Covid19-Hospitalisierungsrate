...
final_comp <- final_df
final_df <- final_df %>% filter(Meldedatum <= "2022-01-30") %>% filter(Meldedatum >= "2021-01-01")
...
png(filename="./Bilder/inzidenz.png", width = 2000, height = 300)
...
impf_bl <- impfung_n %>%
  ...
  filter(Impfdatum >= "2021-01-01") %>%
  mutate(Impfschutz = as.factor(Impfschutz))

impf_ges <- impf_bl %>% group_by(Impfdatum, Impfschutz) %>%
  summarise(Anzahl = sum(Anzahl)) %>%
  group_by(Impfschutz) %>%
  mutate(rolls = cumsum(Anzahl)/N)

png("Bilder/pmf_ges.png", width = 2000, height = 900)
ggplot(impf_ges) +
