...
VOC <- VOC %>%
  mutate(Kalenderjahr = as.factor(KJ),
         Kalenderwoche = as.integer(KW)) %>%
  pivot_wider(names_from = Variante, values_from = Anteil)
```
...
Hospitalisierung ARIMA für einzelne Altersgruppen
...
bay <- final_df %>%
  ...
  arrange(Meldedatum) %>% 
  ungroup() %>%
  mutate(rate = Hospitalisierung/population * 100000) %>%
  select(rate) %>%
  ts()
...
bay_true <- final_comp %>%
  ...
  ungroup() %>%
  mutate(rate = Hospitalisierung/population * 100000) %>%
  select(Meldedatum, rate)


autoplot(bay)
lambda.bc <- BoxCox.lambda(bay)
trans <- BoxCox(bay, lambda = lambda.bc)
autoplot(trans)
acf(trans, lag.max = 40)
pacf(trans, lag.max = 40)
tseries::adf.test(trans)
d <- diff(trans, 1)
autoplot(d)
acf(trans, lag.max = 40)
pacf(trans, lag.max = 40)

aa <- Arima(bay, order = c(6,1,1), lambda = lambda.bc, biasadj = T)

plot(forecast(aa, h = 4)$mean)

cbind(bay_true, fit = forecast(aa, h = 4)$mean)
```
...
bay <- list_of_dfs$Bayern %>% filter(Meldedatum >= "2021-01-04") %>%
  group_by(Kalenderjahr, Kalenderwoche, Meldedatum) %>%
  summarise(Neuerkrankung = sum(Neuerkrankung),
            Hospitalisierung = sum(Hospitalisierung))

bay_var <- left_join(bay, VOC)
bay_neu <- bay_var %>%
  mutate(Neuerk_d = Delta * Neuerkrankung,
         Neuerk_a = Alpha * Neuerkrankung,
         Neuerk_b = Beta * Neuerkrankung,
         Neuerk_g = Gamma * Neuerkrankung,
         Neuerk_o = Omikron * Neuerkrankung) %>%
  filter(!(is.na(KJ)))

ggplot(bay_neu) +
  geom_line(aes(x = Meldedatum, y = Neuerk_d, col = "a")) +
  geom_line(aes(x = Meldedatum, y = Neuerk_a, col = "b")) +
  geom_line(aes(x = Meldedatum, y = Neuerk_b, col = "c")) +
  geom_line(aes(x = Meldedatum, y = Neuerk_g, col = "d")) +
  geom_line(aes(x = Meldedatum, y = Neuerk_o, col = "e")) +
  scale_y_log10() + theme_minimal() +
  scale_colour_manual(name = 'legend', 
         values =c('a'='black','b'='orange', "c" = "red", "d"="green", "e" = "yellow"), 
         labels = c('Delta','Alpha', "Beta", "Gamma", "Omikron"))

BMA::bicreg(x = bay_neu[,-5], y = bay_neu$Hospitalisierung)

for (i in seq(0, 10)) {
  train <- bay_neu[1:(49 + i),]
  test <- bay_neu[(50+i):(51+i),]
  lm_t <- glm(Hospitalisierung ~ Neuerk_a + Neuerk_b + Neuerk_g + Neuerk_d + Neuerk_o +
                Delta + Gamma + Omikron,
             data = train,
             family = poisson())
  plot(lm_t$model$Hospitalisierung, type = "l")
  lines(fitted.values(lm_t), col = "red", type = "p")
  lines(y = test$Hospitalisierung, x = c(50+i,51+i), col = "black")
  lines(y = predict(lm_t, test, type = "response"), x = c(50+i,51+i), col = "green", type = "p")
}
