...
ggplot(p_rate_100) +
  geom_line(aes(x = as.Date(paste("1",Kalenderwoche, Kalenderjahr, sep = "-"), 
                            format = "%u-%W-%Y"), 
                y = hosprate)) + 
  theme_minimal() + xlab("Kalenderwochen") + ylab("Hospitalisierungsrate")
```
...
ggplot(p_rate_abs) +
  geom_line(aes(y = Hospitalisierung, x = Meldedatum)) + 
     xlab("Datum") + ylab("Hospitalisierungen") + theme_minimal()
```
...
p_hosp_inf <- final_df %>%
  ...
  summarise(Hospitalisierung = sum(Hospitalisierung),
            Neuerkrankung = sum(Neuerkrankung)) %>%
  mutate(hosprate = Hospitalisierung / Neuerkrankung)

ggplot(p_hosp_inf) +
  geom_line(aes(y = hosprate, x = Meldedatum)) +
...
ggplot(p_bl_age) +
  geom_line(aes(x = Meldedatum, y = hosprate, col = Bundesland)) + theme_minimal()
```
...
Wir schauen uns nun an, welche Bundesländer unter Umständen zusammen gelegt
werden können, aufgrund von Infektionen, die evtl nicht im eigenen Bundesland
berichtet werden, aber deren Hospitalisierung im eigenen Bundesland sind.
