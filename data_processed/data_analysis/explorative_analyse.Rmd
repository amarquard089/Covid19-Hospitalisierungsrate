...
png(filename="./Bilder/Hospitalisierungsrate_4l.png", width= 2000, height = 900)
ggplot(final_df) +
  geom_point(aes(y = Hospitalisierung/population * 100000, x = Meldedatum, col = Altersgruppe)) +
...
gam_age <- gam(Hospitalisierung ~ Altersgruppe,
               data = p_age_gam,
               family = poisson())
summary(gam_age)
```
...
p_acf <- p_acf %>%
  ...
  ts(deltat = 1/53, start = c(2020, 1))

acf(p_acf, lag.max = 53)
pacf(p_acf, lag.max = 53)


autoplot(p_acf) + xlab("Year") + ylab("Hospitalization normalized")
p_ets_train <- window(p_acf, end = c(2022, 1))
p_ets_test <- window(p_acf, start = c(2022, 2))

ets_mod <- stlm(p_ets_train, method = "arima")
autoplot(ets_mod$stl)
summary(ets_mod$model)
fc <- forecast(ets_mod, h = 1)
accuracy(fc, p_ets_test)
autoplot(forecast(ets_mod, h = 1)) + theme_minimal() + ggtitle(label = "Forecast for Bayern") +
  ylab("Hospitalization")

ets_mod2 <- stlm(p_acf, method = "arima")
autoplot(ets_mod2$stl)
summary(ets_mod2$model)
fc <- forecast(ets_mod2, h = 2)
accuracy(fc, p_ets_test)
autoplot(forecast(ets_mod2, h = 2)) + theme_minimal() + ggtitle(label = "Forecast for Bayern") +
  ylab("Hospitalization")

plot(decompose(p_acf))
```
