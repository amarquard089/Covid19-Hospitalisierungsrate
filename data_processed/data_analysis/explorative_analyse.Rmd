...
trend <- fitted(a)
detrend_ts <- p_acf - trend
plot(detrend_ts)
s_ts <- t(matrix(data = detrend_ts, nrow = 53))
s_part <-colMeans(s_ts, na.rm = T)
plot(as.ts(rep(s_part, 2)))

resi_ts <- p_acf - trend - s_part
...
plot(as.ts(s_part))
s_cor <- diff(s_part, lag = 1)
s_cor_ts <- as.ts(s_cor)
plot(s_cor_ts)
acf(s_cor_ts)
pacf(s_cor_ts)

b <- arima(p_acf, order = c(1,0,2), seasonal = list(order = c(0,1,0), period = 53))
plot(forecast(b))
summary(b)
