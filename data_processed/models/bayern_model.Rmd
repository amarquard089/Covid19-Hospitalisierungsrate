...
bay_gam <- gam(Hospitalisierung ~ Neuerkrankung * Altersgruppe + 
...
gam.check(bay_gam)
plot(bay_gam)
pacf(resid(bay_gam$lme, type = "normalized"))
...
```{r güte des modells}
list_of_dfs$Bayern$population <- 1e5
# wähle ein Jahr zum modellieren
diagnosis_df <- data.frame(rmse = numeric(52), iteration = numeric(52))
forecast_res <- data.frame(Meldedatum = character(52), 
                           Hosp_one_week = numeric(52), 
                           Hosp_two_week = numeric(52))

for (i in seq(0,10)) {
  train_df <- list_of_dfs$Bayern[1: ((53 + i)*6),]
  test_df <- list_of_dfs$Bayern[((53 + i) * 6 + 1):((55 + i)*6),]
  bay_train <- gam(Hospitalisierung ~ Neuerkrankung * Altersgruppe +
                   s(index, bs = "ps", k = 90) + 
                   s(Kalenderwoche, bs = "cc", k = 30) +
                   s(index, bs = "ps", by = Altersgruppe, k = 30) +
                     offset(log(population)),
  ...
  diagnosis_df$rmse[i+1] <- rmse(forecast_l$Hospitalisierung, forecast_l$fit)
...
ggplot(left_join(df, forecast_res, by = "Meldedatum")) +
