...
levels(list_of_dfs$Hamburg$Altersgruppe) <- c("00-14", "00-14", "15-34", "35-79","35-79","80+")
...
hamburg_gam <- gam(Hospitalisierung ~ Neuerkrankung * Altersgruppe +
                 s(index, bs = "ps", k = 90) + 
                 s(Kalenderwoche, bs = "cc", k = 30) +
                 s(index, bs = "ps", by = Altersgruppe, k = 30),
               data = list_of_dfs$Hamburg,
               method = "REML",
               family = "nb",
               knots = list(Kalenderwoche = c(0.5, 53.5)))
summary(hamburg_gam)
gam.check(hamburg_gam)

for (x in unique(list_of_dfs$Hamburg$Altersgruppe)) {
  pdata <- list_of_dfs$Hamburg %>% filter(Altersgruppe == x)
  pdata <- cbind(pdata, predict(hamburg_gam, pdata, type = "response", se = T))
