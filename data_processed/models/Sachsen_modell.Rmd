...
sach_gam <- gam(Hospitalisierung ~ Neuerkrankung * Altersgruppe +
                 s(index, bs = "ps", k = 90) + 
                 s(Kalenderwoche, bs = "cc", k = 30) +
                 s(index, bs = "ps", by = Altersgruppe, k = 30),
               data = list_of_dfs$Sachsen,
               method = "REML",
...
for (x in unique(list_of_dfs$Sachsen$Altersgruppe)) {
  pdata <- list_of_dfs$Sachsen %>% filter(Altersgruppe == x)
  pdata <- cbind(pdata, predict(sach_gam, pdata, type = "response", se = T))
