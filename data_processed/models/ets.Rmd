...
```{r}
p_ets_bay <- final_df %>% filter(Bundesland == "Bayern") %>%group_by(Meldedatum) %>%
  summarise(Hospitalisierung = sum(Hospitalisierung)) 

m_h <- mean(p_ets_bay$Hospitalisierung)
s_h <- sd(p_ets_bay$Hospitalisierung)
...
acf(p_ets_bay, lag.max = 48)
pacf(p_ets_bay, lag.max = 48)

autoplot(p_ets_bay)
p_ets_train <- window(p_ets_bay, end = c(2022,1))
p_ets_test <- window(p_ets_bay, start = c(2022,2))


ets_mod <- stlm(p_ets_bay, method = "arima")
autoplot(ets_mod$stl)
summary(ets_mod$model)
fc <- forecast(ets_mod, h = 2)
accuracy(fc, p_ets_test)
autoplot(forecast(ets_mod, h = 52))
...
plot(decompose(p_ets_bay))
plot(stlf(p_ets_bay, method = "arima", h = 2))
