...
inzidenz_clean <- 
  ...
  mutate(Bundesland = case_when(
    Bundesland == "Berlin" ~ "BerlinundBrandenburg",
    Bundesland == "Brandenburg" ~ "BerlinundBrandenburg",
    Bundesland == "Niedersachsen" ~ "NiedersachsenundBremen",
    Bundesland == "Bremen" ~ "NiedersachsenundBremen",
    Bundesland == "Schleswig-Holstein" ~ "Schleswig-HolsteinundHamburg",
    Bundesland == "Hamburg" ~ "Schleswig-HolsteinundHamburg",
    TRUE ~ Bundesland
  )) %>%
...
```{r}
colnames(hospitalisierung)[which(names(hospitalisierung) == "7T_Hospitalisierung_Faelle")] <- "Hospitalisierung"
...
hospitalisierung_clean <- 
  ...
  mutate(Kalenderjahr = replace(Kalenderjahr, Kalenderwoche == "52" & Kalenderjahr == "2022", "2021")) %>%
  select(Kalenderjahr, Kalenderwoche, Bundesland, Altersgruppe, Hospitalisierung, Berichtsdatum) %>%
...
bevoelkerung <- bevoelkerung %>% filter(age_group != "00+") %>% 
  ...
  select(Bundesland, Altersgruppe, population) %>%
  group_by(Bundesland, Altersgruppe, population) %>% 
...
saveRDS(inz_hosp_w_all_Age, file = "./final_df_new.RDS")
```