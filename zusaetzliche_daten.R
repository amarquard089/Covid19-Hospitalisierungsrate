...
# Hier werden zusätzliche Daten gesammelt, die über Crawling oder manuelle Dateneingabe
# erfasst werden und über die, vom Projektparnter bereitgestellten Daten
# hinaus gehen.
...
bundeslaender_einwohner <- unlist(str_split(str_split(bundeslaender_einwohner, "\t")[[1]], "\n"))
...
bevoelkerungsdichte <- data.frame(bundesland = bundeslaender, 
                                  ...
                                  flaeche_qkm = as.numeric(flaeche))

head(bevoelkerungsdichte)
library(dplyr)
